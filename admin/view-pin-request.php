...
if(isset($_POST['send'])){
	...
	mysqli_query($con,"update pin_request set status='close',remark='approved' where id='$id' limit 1");
	
	echo '<script>window.location.assign("view-pin-request.php?status=1");</script>';	
}
...
if(isset($_POST['cancel'])){
	...
	//updae pin request status
	mysqli_query($con,"update pin_request set status='close',remark='$remark' where id='$id' limit 1");
	
	echo '<script>window.location.assign("view-pin-request.php?status=2");</script>';	
}
                ...
                <div class="row">
                	<div class="col-lg-12">
                	    ...
                	    if(isset($_GET['status']))
                	    {
                	        if($_GET['status']=='1')
                	        {
                	            echo "<div class='alert alert-success'>Pin Request Approved !!!</div>";
                	        }
                	        if($_GET['status']=='2')
                	        {
                	            echo "<div class='alert alert-danger'>Pin Request Declined !!!</div>";
                	        }
                	    }
                	    ?>
                                ...
                                <?php
									...
									if(mysqli_num_rows($query)>0){
										...
										while($row=mysqli_fetch_array($query)){
											...
											//-- get name of user 
											$name=get_user($con,$email);
										?>
                                        	<tr>
                                            	<td><?php echo $i; ?></td>
                                                <td><?php echo $email.' / '.$name['uname']; ?></td>
                                                ...
                                                <form method="post">
                                                	...
                                                	<td><input type="submit" name="send" value="Approve" class="btn btn-primary"></td>
                                                </form>
                                                <td>
                                                    <form method="post">
                                                    	<input type="hidden" name="userid" value="<?php echo $email ?>">
