    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Active User(s) (With Regitered Mobile Number & Name)</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
                	        ...
                	        <tbody>
                	            ...
                	            $counter=1;
                	                ...
                	                while($row=mysqli_fetch_array($query))
                	                {
                	                    ...
                	                    echo "<tr id=row".$row['pin'].">";
                	                    echo "<td>".$counter++."</td>";
                	                     echo "<td>".$row['pin'].' / '.$row['uname'];?>
                	                     <i class='fa fa-tree btn btn-sm btn-info' onclick="show_tree('<?php echo $row['pin'];?>')"></i>
                	                    </td><?php
                	                    //-- get tree details
                	                    
                	                    //-- added on
                	                    $old_date_timestamp = strtotime($row['added_on']);
                                        $new_date = date('d-m-Y', $old_date_timestamp);  
                                        
                                        //-- tree per user
                                        $tree=mysqli_query($con,"select COUNT(id) AS count,level from tree where userid='".$row['pin']."' ");
                                        $tree=mysqli_fetch_array($tree);
                                        if($tree['count']>0)
                                        {$peruser = $tree['count']*20;}
                	                    else
                	                    {$peruser = 0;}
                	                    ...
                	                    if(isset($tree['level']))
                	                    {
                	                        echo "<td>".$tree['level']."</td>";
                	                        echo "<td>".$tree['count']."</td>";
                	                        
                	                    }
                	                    else
                	                    {   
                	                        $tlevel=str_replace('M','', $row['pin']);
                	                        $tle=get_user_tree_level($con,$tlevel);
                	                        echo "<td>".$tle."</td>";
                	                        echo "<td>0</td>";
                	                    ...
                	                    echo "<td>".$peruser."</td>";
                	                    echo "<td>".$new_date."</td>";
                	                     ?><td><input type='button' class='btn btn-sm- btn-danger' value='Delete' onclick="delete_me('<?php echo $row['pin'];?>')"></td><?php
                	                    echo "</tr>";
                	                }
                	            ?>
                	        </tbody>
