...
if(isset($_GET['uname']))
{
    ...
    $company_level = get_company_level($con);
                                ...
                                for ($i = 0; $i < count($pin_array); $i++) 
								{
									...
									//-- check company level and increase user level if increment
                                       echo $total_user_in_company_level = get_user_in_level_from_tree($con,$company_level);
                                        echo $user_in_level = get_total_user_in_level($con,$company_level);
                                        
                                        
                                        if($total_user_in_company_level == $user_in_level)
                                        {
                                            //-- add slab
                                            $colname = 'col'.$company_level;
                                           ...
                                           if(COUNT($active_users)>0)
                                           {
                                               ...
                                               //echo "update user SET current_level=current_level+1 where pin IN ('$active_users')   ";
                                               $in_active_user_level = mysqli_query($con,"update user SET current_level=current_level+1 where pin IN ('$active_users0') ");
                                           }   
                                           
                                           //-- sent transaction to active user for level increament
                                        
                                           
                                           for($j=0; $j<=COUNT($active_users); $j++)
                                                {
                                                    
                                                    $user_level0 = get_user($con,$active_users[$j]);
                                                    $user_level = $user_level0['current_level'];
                                                    $username=$user_level0['pin'];
                                                    ...
                                                    //--transaction
                                                    $transaction0 = mysqli_query($con,"insert into transaction(pin,remark,amount)Values('".$username."','Slab Amount of $user_level Received from Company','".$amt2."')");
                                                    if($transaction0)
                                                    {echo "<br>Transaction Received To - ".$username;}
                                                    
                                                    //-- send income
                                                    //echo "update income SET day_bal=day_bal+$amt2,current_bal=current_bal+$amt2,total_bal=total_bal+$amt2 where userid = '".$username."' ";
                                                    $slab_ledger0 = mysqli_query($con,"update income SET day_bal=day_bal+$amt2,current_bal=current_bal+$amt2,total_bal=total_bal+$amt2 where userid = '".$username."' ");
                                                        if($slab_ledger0)
                                                        {echo "<br>Slab Amount Received To ".$username;}
                                                }
